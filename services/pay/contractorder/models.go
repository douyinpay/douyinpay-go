...
type PrepayRequest struct {
	...
	// 字段含义：交易类型。
	// 格式规则：string[1,16]。
	// 业务规则：取值：APP（APP支付）、JSAPI（JSAPI支付）、MWEB（H5支付）。
	// 示例：APP。
	TradeType string `json:"trade_type"`
	// 字段含义：用户标识。
	// 格式规则：string[1,128]。
	// 业务规则：trade_type=JSAPI时此参数必传，用户在商户appid下的唯一标识（保留）。
	...
	// 格式规则：object。
	// 业务规则：签约并支付下单使用的签约参数。
	// 示例：{"contract_mchid":"6020230307605084","contract_appid":"awofz9bncda6w2w4","plan_id":"67","out_contract_code":"100001256","request_serial":1695,"contract_display_account":"123","contract_notify_url":"https://yoursite.com"}。
...
type PrepayResponse struct {
	// 字段含义：预支付交易会话标识。
	// trade_type为APP和JSAPI时返回，有效期为2小时。
	PrepayId string `json:"prepay_id"`
	// 字段含义：支付跳转链接。
	// trade_type为MWEB时返回，有效期为5分钟。
...
type ContractInfo struct {
	...
	// 字段含义：签约应用ID。
	// 格式规则：string[1,32]。
	// 业务规则：由抖音支付生成的应用ID，全局唯一，必须与appid一致。
	// 示例：awofz9bncda6w2w4。
	ContractAppId string `json:"contract_appid,omitempty"`
	// 字段含义：签约模板ID。
	// 格式规则：string[1,32]。
	// 业务规则：模板ID，联系抖音支付运营申请。
	// 示例：67。
	...
	// 字段含义：请求序列号。
	// 格式规则：int64，禁止使用0开头，纯数字，范围不超过int64的最大值（9223372036854775807）。
	// 业务规则：商户请求签约时的序列号，要求唯一性，主要用于排序，不作为查询条件。
	// 示例：1695。
	RequestSerial int64 `json:"request_serial,omitempty"`
	// 字段含义：用户账户展示名称。
	// 格式规则：string[1,64]，不支持UTF8非3字节编码的字符（如表情符号），请勿传抖音昵称。
	// 业务规则：签约用户的名称，用于页面展示。
	// 示例：123。
