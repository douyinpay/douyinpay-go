...
type PrepayRequest struct {
	// 字段含义：应用ID。
	// 格式规则：string[1,32]。
	// 业务规则：商户在抖音开放平台申请的应用ID，全局唯一。此处请填写网站应用类型的AppID，并确保该AppID与mchid有绑定关系。
	// 示例：awofz9bncda6w2w4。
...
type PrepayResponse struct {
	// 字段含义：二维码链接。
	// 此URL用于生成支付二维码，然后提供给用户扫码支付。code_url有效期为2分钟，失效后需要重新请求该接口以获取新的code_url。注意：code_url并非固定值，使用时按照URL格式转成二维码即可。
